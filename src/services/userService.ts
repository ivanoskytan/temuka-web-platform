...
export async function getUserDetail(id: number) {
    const res = await fetch(`${API_SERVICE_KEY}/api/user/${id}`, {
        method: 'GET',
...
export async function searchUsers(name: string) {
    ...
    const res = await fetch(`${API_SERVICE_KEY}/api/user/search?${params}`, {
        method: 'GET',
        headers: getAuthHeaders(),
    });
    return res.json();
...
export async function followUser(payload: any) {
    const res = await fetch(`${API_SERVICE_KEY}/api/user/follow`, {
        method: 'POST',
        headers: getAuthHeaders(),
...
export async function getFollowers(payload: any) {
    const res = await fetch(`${API_SERVICE_KEY}/api/user/followers`, {
        method: 'GET',
        headers: getAuthHeaders(),
        body: JSON.stringify(payload)
    });
